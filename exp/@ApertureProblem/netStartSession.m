...
params.diskSize = [params.diskSize 3000];
% Generate alpha masks

% determine size
maxPeriod = double(uint16(max(e.textureSize) - max(params.diskSize)));
...
for i = 1:nDiskSizes
    ...
    e.alphaMask(i) = Screen('MakeTexture',win,cat(3,uint8(alphaLum),uint8(alphaBlend)));
