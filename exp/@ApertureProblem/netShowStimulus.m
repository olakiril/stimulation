...
for i = 1:nFrames*stimFrames
    ...
    if frames == 0
        ...
        movOri = getSessionParam(e,'movementOri',conds(k));
        if isnan(movOri);movOri = 0;else diskSize = 3000;end
        phi = getSessionParam(e,'initialPhase',conds(k));
        period = 1 / spatialFreq;
        speed = getSessionParam(e,'speed',conds(k));    % in cycles/sec
        speed = speed * period / refresh/cosd(movOri);  % convert to px/frame & adjust for off axis speed.
...
%     u = mod(phi,period) - period/2;
    u = phi - period/2;
    xInc = -u * sin((orientation+movOri)/180*pi);
    yInc = u * cos((orientation+movOri)/180*pi);
    ...
    % draw grating
    Screen('DrawTexture',win,e.textures(conds(k)),[],destRect,orientation+90+movOri); 
