...
e.textureSize(cond) = getSessionParam(e,'textureSize',cond);
movOri = getSessionParam(e,'movementOri',cond);
if isnan(movOri); movOri = 0;end
...
phi = 2*pi*spatFreq * (1:e.textureSize(cond)*2);
if sinusoidal
...
grat = imrotate(repmat(grat,[size(grat,2),1,1]),movOri,'crop');
c = round(size(grat,1)/2);
hs = e.textureSize(cond)/2;
grat = grat(c - ceil(hs):c+floor(hs)-1,c - ceil(hs):c+floor(hs)-1,:);
